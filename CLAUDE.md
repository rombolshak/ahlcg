...
- **Never invent a string a user will read.** Wording is chosen, not produced in passing: read [docs/voice-and-tone.md](docs/voice-and-tone.md) and run `/wording`, which gets you variants to pick from and writes the choice into `en.json`. This applies to a single button label as much as to a dialog.
...
- **The backend implements authentication and game records only** — create a game, list the ones you are a member of, fetch the most recent. The frontend calls those three from the main menu; the game view itself renders a hardcoded fixture (`@domain/testing/test-game-state`). The SignalR client connects and reports connection state but carries **no game state**. There is no API for game *state* and no deployment pipeline. Do not write code that assumes otherwise.
