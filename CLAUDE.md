...
- **The backend implements authentication and game records only** — create a game, list the ones you are a member of, fetch the most recent. The frontend calls those three from the main menu; the game view itself renders a hardcoded fixture (`@domain/testing/test-game-state`). There is no API for game *state*, no SignalR client, no deployment pipeline. Do not write code that assumes otherwise.
