...
Every run **deletes the screenshots it made last time and uploads the set fresh**, so nothing goes stale: `screenshot upload` upserts on filename, and a renamed story — or one that stopped showing translatable text — would otherwise leave an image nothing ever updates again. The wipe is scoped by the `storybook` label, so screenshots a person uploaded by hand are left alone. It also means auto-tag re-runs against the current image every time, rather than a re-captured screen keeping tags that point at text which has since moved.
