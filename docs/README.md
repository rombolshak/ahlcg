...
The backend implements **authentication and game records** — creating a game, listing the ones you are a member of, and fetching the most recent — nothing more. The frontend calls those from the main menu only; the game view itself renders from a **hardcoded fixture** (`@domain/testing/test-game-state`), not from the server. There is no API for game *state*, no SignalR client, and no deployment pipeline. Do not write code that assumes any of them exist.
