...
@Component({
  ...
  imports: [ThreatAreaComponent, InvestigatorComponent, ControlAreaComponent, ActionsAreaComponent],
...
export class CurrentInvestigatorPanelComponent {
  ...
  readonly assets = computed(() => {
    return this.state.currentInvestigator()?.controlledAssets.map(a => this.state.getAsset(a)) ?? [];
  });

  readonly noThreatsText = this.noThreatsPhraseService.getPhrase(this.state.currentInvestigator);
  ...
  readonly threatsSeverity = this.threatsSeverityService.getThreatsSeverity(this.investigatorFullModel);
}
