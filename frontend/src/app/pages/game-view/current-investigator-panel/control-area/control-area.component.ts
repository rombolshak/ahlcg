...
@Component({
  ...
  host: {
    class: 'flex-1 min-h-0 flex flex-col justify-end gap-3 p-4 outline outline-2 outline-gray-600 rounded relative',
  },
...
export class ControlAreaComponent implements OnInit, OnDestroy {
  ...
  protected readonly emptySlots = computed(() =>
    Object.entries(emptySlots(this.assets(), this.maxSlotsCounts()))
      ...
      .flatMap(([slot, count]) => new Array<AssetSlot>(count).fill(assetSlot.assert(slot))),
  );
  ...
  protected readonly totalPages = computed(() => getTotalPages(this.activeAssets().length, this.passiveAssets().length, this.availableHeight()));
  ...
  protected readonly passiveAssetsToDisplay = computed(() => {
    const slice = slicePassiveAssets(this.activeAssets().length, this.currentPage(), this.availableHeight());
    return this.passiveAssets().slice(slice.start, slice.end);
  });

  private readonly activeAssets = computed(() => this.assets().filter(a => isActive(a)));

  private readonly passiveAssets = computed(() => this.assets().filter(a => !isActive(a)));
