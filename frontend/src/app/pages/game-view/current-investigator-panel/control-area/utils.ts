...
export const emptySlots: (assets: AssetCard[], slotsCount: SlotsCount) => SlotsCount = (assets, counts) => {
  ...
  const takenSlots = assets.flatMap(a => [...convertSlot(a.slot), ...convertSlot(a.additionalSlot)]);
...
export const getTotalPages = (activeAssetsCount: number, passiveAssetsCount: number, availableHeight: number) => {
  const totalNeededRows = Math.ceil((activeAssetsCount + Math.ceil(passiveAssetsCount / 4)) / 3);
  return Math.ceil(totalNeededRows / getAvailableRows(availableHeight));
};

export const sliceActiveAssets = (currentPage: number, availableHeight: number) => {
...
export const slicePassiveAssets = (activeAssetsCount: number, currentPage: number, availableHeight: number) => {
  const pageSize = getPageSize(availableHeight);
  ...
  const start = firstPageSize + (currentPage - firstPageIndex - 1) * pageSize * 4;
  return { start, end: start + pageSize * 4 };
};

const getPageSize = (availableHeight: number) => getAvailableRows(availableHeight) * 3;
const getAvailableRows = (availableHeight: number) => {
  const pagerHeight = 42;
  const rowHeight = 5.5 * 16;
  const gap = 3 * 4;
  return Math.max(Math.floor((availableHeight - pagerHeight + gap) / (rowHeight + gap)), 1);
};
