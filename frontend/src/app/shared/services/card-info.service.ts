...
import { cardInfo, CardInfo, SetInfo } from '../domain/entities/details/card-info.model';
...
export class CardInfoService {
  ...
  public getCardInfo(card: Signal<GameCard | undefined>): Signal<CardInfo | undefined> {
    effect(() => {
  ...
  private loadInfo(setInfo: SetInfo | undefined): void {
    ...
    const strings$ = this.transloco.langChanges$.pipe(switchMap(lang => this.transloco.load(`cards/${setInfo.set}/${setInfo.index}/${lang}`)));
    const traits$ = this.transloco.langChanges$.pipe(switchMap(lang => this.transloco.load(`traits/${lang}`)));
    ...
    sub.subscribe({
      ...
      error: err => {
        console.error(`Error on loading card ${this.setInfoToString(setInfo)}`, err);
        this.cache.update(value => {
          return {
            ...value,
  ...
  private createInfoModel(setInfo: SetInfo, desc: CardDescription) {
    ...
    for (let i = 1; i <= desc.abilities; i++) {
      // @ts-expect-error construct object manually, then validate via arktype
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      model.abilities.push(`cards/${setInfo.set}/${setInfo.index}.a${i.toString()}`);
    }

    const info = cardInfo(model);
    if (info instanceof ArkErrors) {
      console.error(`Error on model creation of card ${this.setInfoToString(setInfo)}`, info.summary);
      return info.throw();
    }
  ...
  private getCardDescription(setInfo: SetInfo) {
    return this.http.get<CardDescription>(`/assets/cards/${setInfo.set}/${setInfo.index}.json`).pipe(
      map((desc: CardDescription) => {
        const model = cardDescription(desc);
        if (model instanceof ArkErrors) {
          console.error(`Error in description of card ${this.setInfoToString(setInfo)}`, model.summary);
          return model.throw();
        }
